...
function MonitorApp(){
    ...
    this.initMonitoring = function(userEmail,userName,refreshInterval,userListCallback) {
        ...
        commentsRef.once('value', function (snapshot) {
            ...
            if(!CURRENT_USER_KEY)
                writeUserDataEncode(userName, userEmail, url);
    ...
    var clearUserSet = function(userSet, item) {
        ...
        for (var key in userSet) {
            if (userSet.hasOwnProperty(key)) {
                console.log("userSet " + key + " " + userSet[key].updatedAt);
                if (userSet[key].updatedAt < item.updatedAt - 10000 && key != item.userEmail) {
                    console.log("OUT OF DATE " + key);
                    delete userSet[key];
                }
            }
        };
